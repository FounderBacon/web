...
interface TrapInfoColumnProps {
  trap: TrapDetail
  tierData: TierData
}

export function TrapInfoColumn({ trap, tierData }: TrapInfoColumnProps) {
  const rarityName = trap.rarity === "mythic" ? "mythic" : trap.rarity
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="relative mx-auto w-full">
        ...
        <AssetImage
          src={weaponIconLarge(trap.icon, "traps")}
          alt={trap.name}
      ...
      <div className="space-y-1">
        <InfoRow label="Rarity" value={trap.rarity} className={`capitalize ${rarityColor}`} />
        <InfoRow label="Element" value={trap.element} className="capitalize" />
        <InfoRow label="Placement" value={trap.placement} className="capitalize" />
        <InfoRow label="Type" value={trap.trapType} className="capitalize" />
        <InfoRow label="Target" value={trap.target} className="capitalize" />
      </div>
