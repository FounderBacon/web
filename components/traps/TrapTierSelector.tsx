...
interface TrapTierSelectorProps {
  trap: TrapDetail
  tier: string
  level: number
  offensive: number
...
export function TrapTierSelector({
  trap,
  tier,
  level,
  offensive,
  ...
  onLevelChange,
  onOffensiveChange,
}: TrapTierSelectorProps) {
  const tierKeys = Object.keys(trap.tiers)
  const activeTd: TierData | undefined = trap.tiers[tier]
    ...
    <div className="mb-4 overflow-hidden border border-border/50">
      ...
      <div className="space-y-2 p-3">
        ...
        <div className={`grid gap-1 ${gridCols(tierKeys.length)}`}>
          {tierKeys.map((t) => {
            const td: TierData | undefined = trap.tiers[t]
